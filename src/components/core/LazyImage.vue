<template>
    <picture v-if="useImageTag">
        ...
        <img
            ...
            :height="height"
            loading="lazy"
            class="transition-opacity ease-in-out duration-150"
            :class="[
                imgClass,
                {
                    'opacity-0': !loaded,
...
<script>
import getSrcSet from '@/utils/img/getSrcSet'
import imageCacheService from '@/utils/img/imageCache'
...
export default {
    ...
    methods: {
        onLoad() {
            if (this.srcsetOptions && this.$refs.img) {
